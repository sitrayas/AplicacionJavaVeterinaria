...
public class ctrl_cliente implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == this.pacientes.btn_guardar_cliente) {
            ...
            conexion.estado_civil=this.pacientes.cb_estado_civil_cliente.getSelectedItem().toString();
            conexion.sexo = this.pacientes.txt_sexo_cliente.getText();
            conexion.estado = this.pacientes.txt_estado_cliente.getText();
            conexion.tipo_identificacion = this.pacientes.cb_tipo_identificacion_cliente.getSelectedIndex();
            conexion.tipo_cliente = this.pacientes.cb_tipo_clientes.getSelectedIndex();
...
//        
//    }
    public void llenarComboTipoCliente() throws SQLException {
        ResultSet rs = conexion.tipoIdentificacion();
        while (rs.next()) {
            this.pacientes.cb_tipo_identificacion_cliente.addItem(rs.getString(1));
        }
    }

    public void llenarComboCliente() throws SQLException {
        ResultSet rs = conexion.tipoCliente();
        while (rs.next()) {
            this.pacientes.cb_tipo_clientes.addItem(rs.getString(1));
        }
    }
}
